...
To determine the optimal configuration for the CaVL-Doc framework, we conducted the five-phase protocol described in Section \ref{sec:protocol}.
...
\begin{table}[h]
    ...
    \midrule
    \textit{Euclidean} \\
    Contrastive Loss & \textbf{1.98} & 27.99 \\
    Triplet Loss & 2.27 & \textbf{22.99} \\
    \addlinespace
...
\paragraph{Analysis} 
The results reveal a divergence based on domain complexity. On \textbf{LA-CDIP}, which focuses on structural layout variance, the \textbf{Contrastive Loss} (Euclidean) achieved the best performance (1.98\% EER). Conversely, on \textbf{RVL-CDIP}, which contains higher semantic noise, the \textbf{Triplet Loss} proved superior (22.99\% EER), likely due to its ability to model relative distances more effectively in a crowded embedding space. 

Based on this, we select \textbf{Contrastive Loss} as the engine for LA-CDIP and \textbf{Triplet Loss} for RVL-CDIP in the final comparison.

The results indicate a divergence in optimal baselines depending on the dataset complexity. However, \textbf{[BEST LOSS]} provided the most consistent stability across both domains and was selected as the fixed engine.
...
\textbf{CaVL-Doc (Best Configuration)} & \textbf{Embedded} & \textbf{1.98} & \textbf{22.99} \\
\bottomrule
\end{tabularx}
