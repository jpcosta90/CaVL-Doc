...
OUT=$(wandb sweep scripts/sweeps/configs/rvlcdip/fine_search/fine_search_contrastive_k3.yaml --project $PROJECT 2>&1)
SWEEP_CONTRASTIVE_K3=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

echo "Registrando subcenter_arcface_k1..."
OUT=$(wandb sweep scripts/sweeps/configs/rvlcdip/fine_search/fine_search_subcenter_arcface_k1.yaml --project $PROJECT 2>&1)
SWEEP_SUBCENTER_ARCFACE_K1=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

echo "Registrando subcenter_cosface_k1..."
OUT=$(wandb sweep scripts/sweeps/configs/rvlcdip/fine_search/fine_search_subcenter_cosface_k1.yaml --project $PROJECT 2>&1)
SWEEP_SUBCENTER_COSFACE_K1=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

echo "Registrando subcenter_cosface_k3..."
OUT=$(wandb sweep scripts/sweeps/configs/rvlcdip/fine_search/fine_search_subcenter_cosface_k3.yaml --project $PROJECT 2>&1)
SWEEP_SUBCENTER_COSFACE_K3=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

echo "Registrando triplet_k3..."
OUT=$(wandb sweep scripts/sweeps/configs/rvlcdip/fine_search/fine_search_triplet_k3.yaml --project $PROJECT 2>&1)
