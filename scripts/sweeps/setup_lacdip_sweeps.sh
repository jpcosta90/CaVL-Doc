...
OUT=$(wandb sweep scripts/sweeps/configs/lacdip/sweep_contrastive.yaml 2>&1)
CONT_AGENT=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

# 5. Triplet (Moved down - Already run)
echo "Creating Triplet Sweep..."
OUT=$(wandb sweep scripts/sweeps/configs/lacdip/sweep_triplet.yaml 2>&1)
