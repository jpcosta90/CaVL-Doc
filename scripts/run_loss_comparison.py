...
def main():
    # 1. Prepara os dados antes de tudo
    prepare_data()

    print(f"Iniciando Bateria de Testes: {len(LOSSES_TO_TEST)} experimentos.")
    print(f"Projeto WandB: {WANDB_PROJECT}")
    
    for i, loss in enumerate(LOSSES_TO_TEST):
        print(f"\n>>> Progresso: {i+1}/{len(LOSSES_TO_TEST)}")
        run_experiment(loss)
