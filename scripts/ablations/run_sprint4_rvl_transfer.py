...
Para cada split, roda dois treinos em paralelo (sequencial):
...
def _download_hf_checkpoint(hub_id: str, cache_dir: Path) -> tuple[Path, dict]:
    """
    Baixa best_siam.pt do HF Hub para cache_dir (se ainda não existir).
    Retorna (caminho_local, config_dict).
    """
...
def main() -> None:
    ...
    p.add_argument("--hf-cache-dir", default=None,
                   help="Diretório local para cache do modelo HF (default: checkpoints/hf_cache/<model-id>)")
    ...
    # Warmup do professor (épocas sem seleção ativa)
    ...
    loss_type = (
        args.loss_type
        or hf_config.get("loss_type")
        or "subcenter_cosface"
    )
    ...
    for split_idx in splits:
        ...
        cmd_direct = _build_cmd(
            ...
            args=args,
            loss_type=loss_type,
            init_checkpoint=None,           # sem init = treino direto
            professor_lr=args.professor_lr,
            warmup_steps=professor_warmup_steps,
        )
        ...
        if not _should_skip(ckpt_root, run_transfer):
            subprocess.run(cmd_transfer, check=True)
