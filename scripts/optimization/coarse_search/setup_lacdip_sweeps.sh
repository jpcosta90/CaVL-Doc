#!/bin/bash
# scripts/sweeps/setup_lacdip_sweeps.sh

PROJECT="CaVL-Doc_LA-CDIP_InternVL3-2B_Sweeps"
export WANDB_PROJECT=$PROJECT

echo "🚀 Initializing LA-CDIP Sweeps on Project: $PROJECT"
echo "--------------------------------------------------"

# 1. ArcFace (Split K=1,2,3)
echo "Creating ArcFace (K=1) Sweep..."
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_arcface_k1.yaml 2>&1)
ARC1_AGENT=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

echo "Creating ArcFace (K=2) Sweep..."
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_arcface_k2.yaml 2>&1)
ARC2_AGENT=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

echo "Creating ArcFace (K=3) Sweep..."
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_arcface_k3.yaml 2>&1)
ARC3_AGENT=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

# 2. CosFace (Split K=1,2,3)
echo "Creating CosFace (K=1) Sweep..."
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_cosface_k1.yaml 2>&1)
COS1_AGENT=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

echo "Creating CosFace (K=2) Sweep..."
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_cosface_k2.yaml 2>&1)
COS2_AGENT=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

echo "Creating CosFace (K=3) Sweep..."
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_cosface_k3.yaml 2>&1)
COS3_AGENT=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

# 3. Circle (Moved up)
echo "Creating CircleFace Sweep..."
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_circle.yaml 2>&1)
...
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_contrastive.yaml 2>&1)
CONT_AGENT=$(echo "$OUT" | grep "wandb agent" | sed 's/.*wandb agent //')

# 5. Triplet (Moved down - Already run)
echo "Creating Triplet Sweep..."
OUT=$(wandb sweep scripts/optimization/coarse_search/configs/lacdip/sweep_triplet.yaml 2>&1)
