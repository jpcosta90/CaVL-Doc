#!/usr/bin/env python3
"""
Treinamento final CaVL-Doc: ArcFace em dois estágios com early stopping.

Fase 1 — Student only (sem professor): treino longo até convergência.
Fase 2 — Com rede teacher (professor ON): fine-tuning a partir do melhor
          checkpoint da fase 1, também com early stopping.

Diferenças em relação ao Sprint 3:
  - Loss fixo: arcface (padrão; pode ser alterado com --loss-type)
  - Sem sweep de múltiplas losses e sem branch OFF
  - Early stopping (--patience) independente da patience do plateau (--lr-patience)
  - Projeto W&B próprio: CaVL-Doc_LA-CDIP_Final_ArcFace
"""
import argparse
import os
import subprocess
...
def main() -> None:
    ...
    p.add_argument("--wandb-project",     default="CaVL-Doc_LA-CDIP_Final_ArcFace")
    ...
    print(f"Early stop patience: {args.patience}  |  LR patience: {args.lr_patience}")
    ...
    for split_idx in splits:
        ...
        run_p1    = f"Final_S{split_idx}_{args.loss_type}_fase1_E{args.phase1_epochs}{suffix}"
